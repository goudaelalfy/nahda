...
<script type='text/javascript'  src='<?php echo base_url();?>js/admin/video/form.js' > </script>
...
if(!isset($video_category_ids)) {
$video_category_ids=		'';
}
if(!isset($video_category_names)) {
...
if($mode!='add')
{
	...
	if($mode=='return')
	{
		...
		$video_category_ids=	$current_row['video_category_ids'];
		$video_category_names=$current_row['video_category_names'];
	...
	else 
	{
		...
		$video_category_names=$video_category_names;
 ...
 * Video object.
 * 
 * Intialize object from Video controller class.
 * @var object.
 */

$this_object= new Video();
...
$("#dv_alias_true").html('');
    $.ajax({  
    type: "POST",  
    url: "<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/video/check_alias_availability/<?php echo $id; ?>",  
    data: "alias="+ alias,  
...
<h2><i class="icon-edit"></i> <?php echo lang('videos')?></h2>
<div class="box-icon">
...
<form class="form-horizontal" id='frm_video_row' name='frm_video_row'  method='post' enctype='multipart/form-data'>
...
<input type="hidden" name=" hdn_banner_files" id=" hdn_banner_files" value="<?php echo $banner_files; ?>"> 
	...
	<a class='btn btn-success' href='<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/video/table'>
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_add)) {
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/video/form/0/add">
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_edit)) {
    ...
    <a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/video/form/<?php echo $id;?>/edit">
    <i class='icon-edit icon-white'></i>  
...
?>		
	...
	<a class='btn btn-danger' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/video/delete/<?php echo $id;?>" onclick="return delete_confirm('<?php echo $this->lang->lang(); ?>')">
...
<div class="control-group">
<label class="control-label" for="focusedInput"><?php echo lang('video_categories'); ?></label>
<div class="controls">
<div style="width:690px">
<input type="hidden" name="page_category_ids" id="page_category_ids" value="<?php echo $video_category_ids; ?>"> 
<input class="input-xlarge focused" type="text" readonly="readonly" name="page_category_names" id="page_category_names" value="<?php echo $video_category_names; ?>" style="width:600px;">
<a class='btn btn-primary' href='javascript:void(0)' onclick="PopupCenter('<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/video/popuppagecategory/<?php echo $id;?>' , '<?php echo $this->lang->line('select_video_categories')?>',400,500);" >::::</a>
