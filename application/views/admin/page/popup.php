...
{
	window.opener.document.getElementById('page_ids').value=page_id;
	window.opener.document.getElementById('page_names').value=page_name;
...
function selectPages() 
{
		...
		for (var i = 0; i < document.frm_popup.elements.length; i++ ) 
	    {
		    ...
		    {		   
	        	...
	        	if(document.frm_popup.elements[i].checked)
	        	{
		        	if(counter==0)
		        	{
		        		page_ids=page_ids+document.frm_popup.elements[i].value;
		        		page_names=page_names+document.frm_popup.elements[i].name;
		        	}
		        	else
		        	{
		        		page_ids=page_ids+','+document.frm_popup.elements[i].value;
		...
		window.opener.document.getElementById('page_ids').value=page_ids;
		window.opener.document.getElementById('page_names').value=page_names;
...
<?php  echo $this->lang->line('btn_cancel'); ?>
	...
	<?php 
            	...
            	if($previous_lnk>0)
            	{
            		echo "<li><a href='".base_url().$this->lang->lang()."/".ADMIN."/page/popup/$previous_lnk'> ".lang('lnk_prev')."</a></li>";
            	}
            	...
            	for ($counter = 0; $counter < $pages; $counter += 1) 
            	{
            		...
            		{
            			echo "<a href='".base_url().$this->lang->lang()."/".ADMIN."/page/popup/$page_no' $class_style>$page_no</a>";
            		}
            	}
            	...
            	if($next_lnk<=$pages)
            	{
            		echo "<a href='".base_url().$this->lang->lang()."/".ADMIN."/page/popup/$next_lnk' >".lang('lnk_next')." </a>";
            	}
            	?>
