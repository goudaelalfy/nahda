...
<script type='text/javascript'  src='<?php echo base_url();?>js/admin/news/form.js' > </script>
...
if(!isset($news_category_ids)) {
$news_category_ids=		'';
}
if(!isset($news_category_names)) {
$news_category_names=	'';
}
...
if($mode!='add')
{
	...
	if($mode=='return')
	{
		...
		$video_link=			$current_row['video_link'];
		...
		$news_category_ids=	$current_row['news_category_ids'];
		$news_category_names=$current_row['news_category_names'];
	...
	else 
	{
		...
		$news_category_names=$news_category_names;
 ...
 * News object.
 * 
 * Intialize object from News controller class.
 * @var object.
 */

$this_object= new News();
...
$("#dv_alias_true").html('');
    $.ajax({  
    type: "POST",  
    url: "<?php echo base_url().$this->lang->lang().'/'.ADMIN; ?>/news/check_alias_availability/<?php echo $id; ?>",  
    data: "alias="+ alias,  
...
<h2><i class="icon-edit"></i> <?php echo lang('newss')?></h2>
<div class="box-icon">
...
<form class="form-horizontal" id='frm_news_row' name='frm_news_row'  method='post' enctype='multipart/form-data'>
...
<input type="hidden" name=" hdn_banner_files" id=" hdn_banner_files" value="<?php echo $banner_files; ?>"> 
	...
	<a class='btn btn-success' href='<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/news/table'>
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_add)) {
	...
	<a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/news/form/0/add">
...
if($this_object->user_screen_privielge_allowed($this_object->screen_id, $this_object->privielge_edit)) {
    ...
    <a class='btn btn-info' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/news/form/<?php echo $id;?>/edit">
    <i class='icon-edit icon-white'></i>  
...
?>		
	...
	<a class='btn btn-danger' href="<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/news/delete/<?php echo $id;?>" onclick="return delete_confirm('<?php echo $this->lang->lang(); ?>')">
...
<div class="control-group">
<label class="control-label" for="focusedInput"><?php echo lang('video_link'); ?></label>
<div class="controls">
	<input class="input-xlarge focused" type="text" <?php echo $readonly; ?> name="video_link" id="video_link" value="<?php echo $video_link; ?>" style="width:600px;">

<div id="dv_alias_true"></div>
<div class="dv_error"  id="dv_video_link_false" style="display:none;"><?php echo lang('video_link_false'); ?></div>
...
<div class="control-group">
<label class="control-label" for="focusedInput"><?php echo lang('news_categories'); ?></label>
<div class="controls">
<div style="width:690px">
<input type="hidden" name="page_category_ids" id="page_category_ids" value="<?php echo $news_category_ids; ?>"> 
<input class="input-xlarge focused" type="text" readonly="readonly" name="page_category_names" id="page_category_names" value="<?php echo $news_category_names; ?>" style="width:600px;">
<a class='btn btn-primary' href='javascript:void(0)' onclick="PopupCenter('<?php echo base_url().$this->lang->lang().'/'.ADMIN;?>/news/popuppagecategory/<?php echo $id;?>' , '<?php echo $this->lang->line('select_news_categories')?>',400,500);" >::::</a>
...
<div class="control-group">
<label class="control-label" for="focusedInput"><?php echo lang('news_images'); ?></label>
<div class="controls">
<?php if($id!=0) {?>
 <a class='btn btn-primary' href='javascript:void(0)' onclick="PopupCenter('<?php echo base_url().$this->lang->lang()."/".ADMIN."/news/uploads/$id"; ?>' , '<?php echo $this->lang->line('news_images'); ?>.',1000,1000);">:::</a>
 <?php }?>
</div>
</div>	
