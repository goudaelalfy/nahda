	...
	$(document).ready(function(){
		...
		{ 
           var ID=$(".message_box:last").attr("id");
			...
			$.post("<?php echo base_url().$this->lang->lang(); ?>/writer/contentajax/<?php echo $current_alias; ?>?action=get&last_msg_id="+ID,
...
if(isset($writer_title)) {
echo "<li class='first' >
                  <div class='first last'>
                    <div class='first'>
                      <div class='first'><img class='first last' src='".base_url().$writer_banner_file_selected."' alt='$writer_title' width='130px' height='100px'/></div>
                      <div class='last'>
                        <h2 class='first'>$writer_title</h2>
...
$page_rows=$this->Page_model->get_by_writer($writer_id, 20);
if(isset($page_rows)) {
	$page_rows_counter=1;
		...
		$page_writer_alias=$page_row->writer_alias;
      	...
      	if($page_rows_counter==1) {
		   		$page_rows_li_style="class='first'";
		   	}  else if($page_rows_counter==20) {
		   		$page_rows_li_style="class='last zebra'";
		   	}  else if($page_rows_counter%2==0) {
		   		$page_rows_li_style="class='zebra'";
		   	} else {
		   		$page_rows_li_style='';
		   	}
	   	
      	echo "<li $page_rows_li_style>
                  <div class='first last'>
                    <div class='first'>
                      <div class='first'><a class='first last' href='$full_link_url'><img class='first last' src='".base_url().$banner_file_selected."' alt='$title' width='130px' height='100px'/></a></div>
                      <div class='last'>
                        <h2 class='first'><a class='first last' href='$full_link_url'>$title</a></h2>
                        <div><span class='first'>".lang('writer').":</span> <a class='last' href='$page_writer_full_link_url'>$page_writer_name</a></div>
                  ...
                  <div>
                    <div>
                      ...
                      <div>
                        <h2><a href='$full_link_url'>$title</a></h2>
                        <div><span>".lang('writer').":</span> <a href='$page_writer_full_link_url'>$page_writer_name</a></div>
                        <div class='articleTime'>الخميس, 12 شوال 1433 هجريا - 30 أغسطس 2012 ميلاديا</div>
                        <p>$page_brief ..</p>
                      </div>
                    </div>
                    <div class='readMoreContainer'><a href='$full_link_url'>". $this->lang->line('more')."</a></div>
