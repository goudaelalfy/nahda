...
{ 	
		...
		if(isset($_POST['smt_save'])) {
			if($id!=0) {
			 ...
			 if($_FILES['icon']['name']!="") {
					...
					/**
					 * Add link file, to solve conflict if i upload banner and link as image, will take
					 * same name.
					 */	
					$name_file_timestamp=strtotime($current_date).'link';
	...
	*/
	public function aliaspopup($controller_name)
	{
		...
		if($controller_name=='pagecat') {
		 	$data['rows'] = $this->Pagecat_model->get_all();
			$this->load->view('admin/menu_link/popup', $data);
		 }
		if($controller_name=='pagetype') {
		 	$data['rows'] = $this->Pagetype_model->get_all();
			$this->load->view('admin/menu_link/popup', $data);
		 }
		if($controller_name=='categories') {
		 	$data['rows_pagetype'] = $this->Pagetype_model->get_all();
		 	$data['rows_pagecat'] = $this->Pagecat_model->get_all();
		 	
			$this->load->view('admin/menu_link/popup_pagetypecat', $data);
		 }
	}
