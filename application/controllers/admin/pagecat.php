...
/**
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Pagecat_model->get_count_by_alias($this->table,$id,$alias);
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					$data['current_row'] = array(
				         	...
				         	$duplicated_id=$this->Pagecat_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/pagecat/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
