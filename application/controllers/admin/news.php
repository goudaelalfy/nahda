 ...
 *
 * Contains controller class of the news table.
 *
...
/**
 * Controller class of the news table.
 *
 * This is the controller class of the news table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	/**
	 * store this controller news screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller news table name.
	 *
	 * @var string
	 * @access public
	 */
	...
	*/
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/news/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->News_model->get_count($this->table);
		...
		$data['rows'] = $this->News_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/news/table", $data);
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->News_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->News_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->News_model->get_by_id($this->table, $id);
			//------------------------ News Categories ----------------------------------------
			$news_categories_rows = $this->News_model->get_news_categories_by_id($id);
			$data['news_category_ids']='';
			$data['news_category_names']='';			
			$counter=0;
			foreach($news_categories_rows as $news_categories_rows)
			{			
				$news_category_id=$news_categories_rows->page_category_id;
				if($this->lang->lang()=='ar') {
					$news_category_name=$news_categories_rows->name_ar;
				} else {
					$news_category_name=$news_categories_rows->name;
				}
				if($counter==0)
				{
					$data['news_category_ids']=$data['news_category_ids'].$news_category_id;
					$data['news_category_names']=$data['news_category_names'].$news_category_name;
				}
				else
				{
					$data['news_category_ids']=$data['news_category_ids'].','.$news_category_id;
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					$data['current_row'] = array(
								'id' => $id ,
				               	'alias' => $_POST['alias'] ,
								'news_category_ids' => $_POST['page_category_ids'] ,
								'news_category_names' => $_POST['page_category_names'] ,
				         	...
				         	$duplicated_id=$this->News_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/news/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/banner/news/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["banner_file"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$banner_files = '/added/uploads/banner/news/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumbs= '/added/uploads/banner/news/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$banner_file_selected = '/added/uploads/banner/news/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumb_selected= '/added/uploads/banner/news/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$banner_image_thumbs = '/added/uploads/banner/news/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$banner_image_thumbs = $_POST['hdn_banner_image_thumbs'].',,,'.'/added/uploads/banner/news/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_banner_files']=='') {
								$banner_files = '/added/uploads/banner/news/'.$name_file_timestamp.'.'.$extension;
							} else {
								$banner_files = $_POST['hdn_banner_files'].',,,'.'/added/uploads/banner/news/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($id==0){
					$this->News_model->insert($this->table, $data['current_row']);
					$id=$this->News_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->News_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				...
				if(!file_exists(getcwd().'/added/uploads/news/images/'.$id)) {
						mkdir(getcwd().'/added/uploads/news/images/'.$id, 0777);
				}
					
				//Insert news categoriess.
				$news_category_ids = $_POST['page_category_ids'];
				$news_category_ids = $news_category_ids;
				$news_category_ids = explode(",", $news_category_ids);
				$news_category_data = array();
				foreach($news_category_ids as $news_category_id) {
					if($news_category_id!=0) {
					$news_category_data[]=array('news_id' => $id,'page_category_id' => $news_category_id);
					}
				}
				$this->Newscats_model->insert($id,$news_category_data);

				redirect(base_url().$this->lang->lang()."/".ADMIN."/news/form/$id/view");
			}
		}
	...
	*/
	public function popuppagecategory($id)
	{
		...
		$news_categories_rows = $this->News_model->get_news_categories_by_id($id);
		$data['page_categories_rows'] = $news_categories_rows;
	...
	function uploads($id)
	{
		$this->session->set_userdata('news_uploads_folder','/added/uploads/news/images/'.$id.'/');
		$this->load->view('admin/news/uploader');
	}
	function uploadstart()
	{
		$news_uploads_folder=$this->session->userdata('news_uploads_folder');
		
		error_reporting(E_ALL | E_STRICT);
		require(getcwd().'/added/uploader/server/php/UploadHandler.php');
		$upload_handler = new UploadHandler($news_uploads_folder);
	}
}