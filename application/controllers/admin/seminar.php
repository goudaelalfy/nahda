 ...
 *
 * Contains controller class of the seminar table.
 *
...
/**
 * Controller class of the seminar table.
 *
 * This is the controller class of the seminar table. between model and view, in MVC design pattern.
 *
 * @package		admin
 * @category	controller
 * @author		Eng Gouda Elalfy <goudaelalfy@hotmail.com>
...
{ 	
	/**
	 * store this controller seminar screen id.
	 *
	 * @var int
	 * @access public
	 */
	...
	/**
	 * store this controller seminar table name.
	 *
	 * @var string
	 * @access public
	 */
	...
	*/
	public function __construct()
	{
		...
		$this->load->model('Seminarcats_model', 'Seminarcats_model' , True);
	...
	{
		redirect(base_url().$this->lang->lang()."/".ADMIN."/seminar/table");
	}
	...
	public function table($page=1, $order=null, $order_type=null)
	{
		...
		$rows_count = $this->Seminar_model->get_count($this->table);
		...
		$data['rows'] = $this->Seminar_model->get_all_display_paging($this->table, $start, $per_page, $this->table_fields_to_display, $order, $order_type); 
        
		
		$this->load->view("admin/seminar/table", $data);
	}
		...
		if(!$this->user_screen_privielge_allowed($this->screen_id, $this->privielge_delete)) {
			...
			if(isset($_POST['chk_current_row'])) {
				...
				foreach($del as $del_id) {
					...
					$this->Seminar_model->update($this->table, $del_id, $data);
	...
	{
		$alias = $_POST['alias'] ;
		$alias_count=$this->Seminar_model->get_count_by_alias($this->table,$id,$alias);
	...
	{ 		
        ...
        if($id!=0){	
			$data['current_row'] = $this->Seminar_model->get_by_id($this->table, $id);
			//------------------------ Seminar Categories ----------------------------------------
			$seminar_categories_rows = $this->Seminar_model->get_seminar_categories_by_id($id);
			$data['seminar_category_ids']='';
			$data['seminar_category_names']='';			
			$counter=0;
			foreach($seminar_categories_rows as $seminar_categories_rows)
			{			
				$seminar_category_id=$seminar_categories_rows->page_category_id;
				if($this->lang->lang()=='ar') {
					$seminar_category_name=$seminar_categories_rows->name_ar;
				} else {
					$seminar_category_name=$seminar_categories_rows->name;
				}
				if($counter==0)
				{
					$data['seminar_category_ids']=$data['seminar_category_ids'].$seminar_category_id;
					$data['seminar_category_names']=$data['seminar_category_names'].$seminar_category_name;
				}
				else
				{
					$data['seminar_category_ids']=$data['seminar_category_ids'].','.$seminar_category_id;
		...
		if(isset($_POST['smt_save'])) {
			...
			if($this->alias_redundency($id)) {
					$data['current_row'] = array(
								'id' => $id ,
				               	'alias' => $_POST['alias'] ,
								'seminar_category_ids' => $_POST['page_category_ids'] ,
								'seminar_category_names' => $_POST['page_category_names'] ,
				         	...
				         	$duplicated_id=$this->Seminar_model->get_id_by_alias($this->table,$_POST['alias']);
				            $data['error']= lang('alias_redundency_error')."<a  href='".base_url().$this->lang->lang()."/".ADMIN."/seminar/form/$duplicated_id/view' >".$this->lang->line('click_here_link')."</a>";
				...
				$uplad_path_file=getcwd().'/added/uploads/banner/seminar/' . $name_file_timestamp.'.'.$extension;
				...
				//if ($userfile_type!='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
				{
					if(move_uploaded_file($_FILES["banner_file"]["tmp_name"], $uplad_path_file))
					{
						...
						if($id==0) {
							$banner_files = '/added/uploads/banner/seminar/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumbs= '/added/uploads/banner/seminar/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							$banner_file_selected = '/added/uploads/banner/seminar/'.$name_file_timestamp.'.'.$extension;
							$banner_image_thumb_selected= '/added/uploads/banner/seminar/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							...
							$banner_image_thumbs = '/added/uploads/banner/seminar/'.$name_file_timestamp.'_thumb'.'.'.$extension;
								...
								$banner_image_thumbs = $_POST['hdn_banner_image_thumbs'].',,,'.'/added/uploads/banner/seminar/'.$name_file_timestamp.'_thumb'.'.'.$extension;
							}
							
							if($_POST['hdn_banner_files']=='') {
								$banner_files = '/added/uploads/banner/seminar/'.$name_file_timestamp.'.'.$extension;
							} else {
								$banner_files = $_POST['hdn_banner_files'].',,,'.'/added/uploads/banner/seminar/'.$name_file_timestamp.'.'.$extension;
							} 
				...
				if($id==0){
					$this->Seminar_model->insert($this->table, $data['current_row']);
					$id=$this->Seminar_model->get_max_id($this->table);
					//----------User Histroy Row ------------------//
					...
					$this->Seminar_model->update($this->table, $id, $data['current_row']);
					//----------User Histroy Row ------------------//
					$this->User_history_model->insert($this->session->userdata('user_session')->id,$this->screen_id,2,$current_date,$id);
					//---------------------------------------------//
					$this->session->set_userdata('message_session',lang('saved_successfully'));
				...
				if(!file_exists(getcwd().'/added/uploads/seminar/images/'.$id)) {
						mkdir(getcwd().'/added/uploads/seminar/images/'.$id, 0777);
				}
					
				//Insert seminar categoriess.
				$seminar_category_ids = $_POST['page_category_ids'];
				$seminar_category_ids = $seminar_category_ids;
				$seminar_category_ids = explode(",", $seminar_category_ids);
				$seminar_category_data = array();
				foreach($seminar_category_ids as $seminar_category_id) {
					if($seminar_category_id!=0) {
					$seminar_category_data[]=array('seminar_id' => $id,'page_category_id' => $seminar_category_id);
					}
				}
				$this->Seminarcats_model->insert($id,$seminar_category_data);

				redirect(base_url().$this->lang->lang()."/".ADMIN."/seminar/form/$id/view");
			}
		}
        
		$this->load->view('admin/seminar/form',$data);
	}
	...
	*/
	public function popuppagecategory($id)
	{
		...
		$seminar_categories_rows = $this->Seminar_model->get_seminar_categories_by_id($id);
		$data['page_categories_rows'] = $seminar_categories_rows;
	...
	function uploads($id)
	{
		$this->session->set_userdata('seminar_uploads_folder','/added/uploads/seminar/images/'.$id.'/');
		$this->load->view('admin/seminar/uploader');
	}
	function uploadstart()
	{
		$seminar_uploads_folder=$this->session->userdata('seminar_uploads_folder');
		
		error_reporting(E_ALL | E_STRICT);
		require(getcwd().'/added/uploader/server/php/UploadHandler.php');
		$upload_handler = new UploadHandler($seminar_uploads_folder);
	}
}