	...
	*/
	...
	{	
		...
		if(count($row)>0) {
			...
			$writer_row=$this->Writer_model->get_by_id('',$writer_id);
			if(count($writer_row)>0) {
			$data['page_writer_row']=$writer_row;
			}			
		$data['other_writer_article_rows']=$this->Page_model->get_by_writer($writer_id, 10);
		$data['page_selected_rows']=$this->Page_selected_model->get_all(15);
		
		$page_id=$row->id;
		$data['page_related_rows']=$this->Page_model->get_page_pages_by_id($page_id);
		
		$data['writer_rows']=$this->Writer_model->get_all(50);
	...
	{	
		...
		$this->load->view("website/page/search", $data);
	}
